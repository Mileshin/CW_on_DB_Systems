...
CREATE OR REPLACE TRIGGER drivers_change
BEFORE INSERT OR UPDATE ON DRIVERS FOR EACH ROW
DECLARE
   id_d NUMBER;
BEGIN
  SELECT POSITION INTO id_d FROM
	PEOPLE INNER JOIN  POSITIONS
	ON PEOPLE.POSITION=POSITIONS.ID
	WHERE PEOPLE.ID = :NEW.ID_PEOPLE AND POSITIONS.NAME='Водитель';
...
BEGIN
  SELECT ID INTO id_d FROM POSITIONS WHERE NAME='Водитель';
  select ID INTO id_p from DRIVERS where :NEW.ID=ID_PEOPLE AND
	EXISTS select ID from DRIVERS where :NEW.ID=ID_PEOPLE;
